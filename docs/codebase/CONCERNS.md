...
| medium | Desktop file picking may behave differently under the Tauri webview because `showOpenFilePicker()` has no runtime-failure fallback and the `<input type="file">` cancel path relies on browser-specific behavior | `src/services/fileService.ts`, `docs/diataxis/how-to/build-windows-tauri.md` | Desktop imports may fail or hang in packaged builds despite working in the browser | Smoke-test quiz, flashcard, and summary imports in the packaged Tauri app; add a Tauri-safe fallback if issues appear |
...
| `package.json` and `src-tauri/` | Mix application scripts, Android commands, and desktop bundling/toolchain config | Highest churn in scan output plus new wrapper surface | Treat packaging/tooling edits as release-affecting changes and verify commands explicitly |

### 6) `[ASK USER]` Questions

1. [ASK USER] Do you want the “platform-specific code only in `fileService.ts`” rule preserved as a hard boundary, or should native back-button handling be accepted as an explicit exception?
2. [ASK USER] Is iframe-based PDF rendering the intended long-term behavior, or do you want the code brought in line with the `pdfjs-dist` design references?
